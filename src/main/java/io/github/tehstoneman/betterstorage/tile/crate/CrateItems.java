...
public class CrateItems
{
	...
	private final NavigableMap< Integer, ItemStack >	weightedMap			= new TreeMap< >();
	...
	{
		final ItemStack stack = itemsMap.get( item );
	...
	public List< ItemStack > getRandomStacks( int amount )
	{
		...
		final List< ItemStack > stacks = new ArrayList< >( amount );
		for( final ItemStack stack : getRandomStacks() )
		{
			stacks.add( stack );
			if( stacks.size() >= amount )
				break;
		}
		return stacks;
	}
	...
	private NavigableMap< Integer, ItemStack > getWeightedMap()
	{
		if( weightedMapDirty )
		{
			weightedMap.clear();
			int stacks = 0;
			for( final ItemStack stack : getItems() )
	...
	private static class ItemsIterator implements Iterator< ItemStack >
	{
		...
		public ItemStack next()
		{
			...
			if( !picked.isEmpty() )
				for( final int p : picked )
					if( p > index )
						break;
					else
						index++;
			picked.add( index );
			final Map.Entry< Integer, ItemStack > entry = map.ceilingEntry( index );
			final ItemStack stack = entry.getValue();
			final int maxStackSize = stack.getMaxStackSize();
			final int amount = index != entry.getKey() ? maxStackSize : ( stack.stackSize - 1 ) % maxStackSize + 1;
