...
public class ContainerCrate extends ContainerBetterStorage
{
	...
	public ContainerCrate( EntityPlayer player, InventoryCratePlayerView inventory )
	{
		...
		final CratePileData data = inventory.data;
	...
	public void detectAndSendChanges()
	{
		...
		final CratePileData data = playerView.data;
	...
	public ItemStack transferStackInSlot( EntityPlayer player, int slotId )
	{
		...
		if( slotStack == null )
			return null;
		final ItemStack stackBefore = slotStack.copy();

		// If slot is in the container inventory, try to transfer the item to the player.

		if( slotId < getColumns() * getRows() )
		{
			Logger.getLogger( Constants.modId ).info( "Columns " + getColumns() + " : Rows " + getRows() );
			final int count = slotStack.stackSize;
			final boolean success = mergeItemStack( slotStack, playerView.getSizeInventory(), inventorySlots.size(), true );
			final int amount = count - slotStack.stackSize;
		...
		{
			final boolean success = playerView.canFitSome( slotStack );
			final ItemStack overflow = playerView.data.addItems( slotStack );
			slot.putStack( overflow );
			// Send slot contents to player if it doesn't match the calculated overflow.
			( (EntityPlayerMP)player ).connection.sendPacket( new SPacketSetSlot( player.openContainer.windowId, slotId, overflow ) );
			if( !success )
				return null;
		}
