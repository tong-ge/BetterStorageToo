...
public class ContainerCraftingStation extends ContainerBetterStorage
{
	...
	protected void setupInventoryContainer()
	{
		...
		for( int y = 0; y < getRows(); y++ )
			for( int x = 0; x < getColumns(); x++ )
				addSlotToContainer( new SlotBetterStorage( this, inventory, 18 + x + y * getColumns(), 8 + x * 18, 76 + y * 18 ) );
	}
	...
	public void detectAndSendChanges()
	{
		...
		final int outputIsReal = inv.outputIsReal ? 1 : 0;
		// sendUpdateIfChanged(0, outputIsReal, lastOutputIsReal);
		lastOutputIsReal = outputIsReal;

		final int progress = inv.currentCrafting != null ? Math.min( inv.progress, inv.currentCrafting.getCraftingTime() ) : 0;
